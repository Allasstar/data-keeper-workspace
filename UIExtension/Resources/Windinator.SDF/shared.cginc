...
fixed4 fragFunctionRaw(float2 uv, float2 worldPosition, float4 color, float dist, float2 position, float2 halfSize, float2 normal)
{
    ...
    // Adjust outline and shadow distances to be within the shape
    float outlineDist = dist + _OutlineSize; // Adjust distance for inner outline
    float shadowDist = dist - _ShadowSize;   // Adjust distance for inner shadow
    ...
    // Mix the graphic, outline, and shadow based on alpha values
    effects = lerp(graphic, shadow, _ShadowSize > _OutlineSize ? shadowAlpha : 0);
    effects = lerp(effects, outline, _OutlineSize > 0 ? outlineAlpha : 0);

    // Ensure the final alpha combines all effects correctly
    effects.a *= max(graphicAlpha, max(outlineAlpha, shadowAlpha));
    effects.a *= _Alpha;

    // Unity UI clipping and alpha clipping
